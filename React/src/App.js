...
function App() {
  ...
  // let isConditional2 = useMemo(() => isConditional, [isConditional])

  const calculateCustomSummary = useMemo(() => {
    debugger;
    return options => salesCalculateCustomSummary(options, isConditional)
  }, [isConditional]);
  ...
  const dataSource = useMemo(() => { 
    return {
      fields: [{
        ...
        // calculateCustomSummary: (options) => salesCalculateCustomSummary(options, isConditional)
      },{
        caption: "Approved",
  ...
  function onValueChanged({value}) {
    let ds = pivotGridRef.current.instance.getDataSource()
    setIsConditional(value)
    // isConditional  = value
    ds.reload()
  }

  const pivotGridRender = useMemo(() => <PivotGrid id="sales"
      ref={pivotGridRef}
...
function salesCalculateCustomSummary(options, isConditional) {
  switch (options.summaryProcess) {
    case "start":
        options.totalValue = { conditionalVal: 0, rawVal: 0, count: 0 };
        break;
